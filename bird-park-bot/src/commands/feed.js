...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            const feedResult = await this.processFeedingResult(birdInfo, food, preference, interaction.user);
    ...
    findBirdInZoo(birdName) {
        const zooState = zooCommand.getZooState();
    ...
    // 餌やり結果処理
    async processFeedingResult(birdInfo, food, preference, user) {
        const results = {
            favorite: {
                effect: '大喜び',
    ...
    // 餌やり可能な鳥のリスト取得（他のコマンドから使用）
    getAvailableBirdsForFeeding() {
        const zooState = zooCommand.getZooState();
        const availableBirds = [];

        for (const area of ['森林', '草原', '水辺']) {
            zooState[area].forEach(bird => {
                const cooldown = this.checkFeedingCooldown(bird, 'check');
                if (cooldown.canFeed) {
                    availableBirds.push({
                        name: bird.name,
                        area,
